...
async function deleteEmployment(data, token, facultyId) {
  try {
    if (token) {
      try {
        const response = await external_axios_default().delete("http://localhost:3001/api" + "/faculty/basic-info/" + facultyId + "/employment", {
          headers: {
            Authorization: `Bearer ${token}`
          },
          data: {
...
async function addWorkExp(data, token) {
  ...
  try {
    if (token) {
      ...
      try {
        const response = await external_axios_default().post("http://localhost:3001/api" + "/faculty/basic-info/add/work-exp", bod, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
...
async function addEmployment(data, token, facultyId) {
  try {
    if (token) {
      ...
      try {
        const response = await external_axios_default().post("http://localhost:3001/api" + "/faculty/basic-info/add/employment", bod, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
...
async function deleteWorkExp(data, token) {
  ...
  try {
    if (token) {
      try {
        const response = await external_axios_default().delete("http://localhost:3001/api" + "/faculty/basic-info/" + facultyId + "/work-exp", {
          headers: {
            Authorization: `Bearer ${token}`
          },
          data: {
